...
async function wakeDatabase() {
  await retryAsync(() => prisma.$queryRaw`SELECT 1`, {
    retries: 2,
    delayMs: 700,
    label: "Guest database wake-up",
  });
}

async function cleanupExpiredGuests(now: Date) {
  try {
    await retryAsync(
      () =>
        ...
        }),
      {
        retries: 1,
        delayMs: 700,
        label: "Expired guest workspace cleanup",
      },
    );
    ...
    await retryAsync(
        ...
        }),
      {
        retries: 1,
        delayMs: 700,
        label: "Expired guest user cleanup",
      },
    );
  } catch (error) {
...
export async function GET(req: Request) {
  try {
    ...
    await cleanupExpiredGuests(now);

    const result = await retryAsync(
      () => {
        const createdAt = new Date();
        const expiresAt = new Date(
          createdAt.getTime() + GUEST_DURATION_MS,
        );
          ...
          async (tx) => {
            ...
            const sprint = await tx.sprint.create({
              data: {
                ...
                goal:
                  "Finish the core launch flow and clean up the dashboard experience.",
                status: "ACTIVE",
                startDate: createdAt,
                endDate: new Date(
            ...
            await tx.activityLog.createMany({
              data: [
                ...
                },
                {
                  ...
                  description:
                    "Sprint report generated for demo analytics.",
                  metadata: {
      ...
      },
      {
        retries: 2,
        delayMs: 1_000,
        label: "Guest demo creation",
      },
    );

    const redirectUrl = new URL(
      `/dashboard/${result.workspaceId}`,
      req.url,
    );
